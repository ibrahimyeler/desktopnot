...
export const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => {
    return (
      <tr
        ref={ref}
          className={cn("border-b border-[#00ff88]/20 transition-colors hover:bg-[#00ff88]/10 data-[state=selected]:bg-[#00ff88]/20", className)}
        {...props}
      />
    );
  }
  ...
  ({ className, ...props }, ref) => {
    return (
      <th
        ref={ref}
          className={cn(
          "h-12 px-4 text-left align-middle font-medium text-[#00ff88] [&:has([role=checkbox])]:pr-0",
          className
        )}
        {...props}
      />

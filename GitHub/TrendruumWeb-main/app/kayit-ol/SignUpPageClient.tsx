...
import { EnvelopeIcon, LockClosedIcon, EyeIcon, EyeSlashIcon, XMarkIcon, UserIcon, ArrowLeftIcon } from '@heroicons/react/24/outline';
...
const SignUpPageClient = () => {
  ...
  const handleVerifyCode = async () => {
    try {
      ...
      const userPassword = localStorage.getItem('pendingUserPassword'); // Şifreyi de saklamış olmamız gerekiyor

      if (!userEmail) {
        throw new Error('Email bilgisi bulunamadı');
      }
      ...
      if (response.ok && data.meta?.status === 'success') {
        ...
        // Email doğrulama başarılı, şimdi otomatik login yap
        if (userPassword) {
          await performAutoLogin(userEmail, userPassword);
        } else {
          // Şifre yoksa sadece ana sayfaya yönlendir
          router.push('/');
        }
  ...
  const handleResendCode = async () => {
    try {
      ...
      const response = await fetch(`${API_V1_URL}/auth/resend-code`, {
        ...
        body: JSON.stringify({
          email: userEmail,
          device_name: 'web' // Device name eklendi
        })
      });
      ...
      if (response.ok && data.meta?.status === 'success') {
        setVerificationTimer(600);
        setVerificationCode(['', '', '', '', '', '']);
        toast.success('Yeni doğrulama kodu email adresinize gönderildi.');
      } else {
  ...
  const calculatePasswordStrength = (password: string) => {
    ...
    // Email ile aynı şifre kontrolü
    if (password === email) {
      setPasswordStrength({ score: 0, message: 'Zayıf' });
      setPasswordError('Şifreniz email adresinizle aynı olamaz');
      return;
    }
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 relative">
          ...
          <h2 className="text-2xl font-semibold mb-4 text-black">Email Doğrulama</h2>
      ...
      <div className="md:hidden min-h-screen bg-gradient-to-br from-orange-100 via-orange-200 to-red-200">
        ...
        <div className="px-4 py-6">
          ...
          <div className="relative z-10 max-w-md mx-auto w-full">
            ...
            <button
              type="submit"
              disabled={!email || !password || !firstName || !lastName || !dataProcessing || !emailConsent || !privacyPolicy || !!passwordError}
              className={`w-full py-3 px-4 rounded-lg font-semibold text-lg transition-colors ${
                !email || !password || !firstName || !lastName || !dataProcessing || !emailConsent || !privacyPolicy || !!passwordError

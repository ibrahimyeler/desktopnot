...
export const useSearchStickyManagement = () => {
  useEffect(() => {
    let isFixed = false;
    ...
    const fixStickyParents = () => {
      ...
      const mainElement = document.querySelector('main[data-page="search"]');
      ...
      parentsToFix.forEach(el => {
        ...
        if ((el.tagName === 'MAIN' || el.hasAttribute('data-page')) && computed.flex !== 'none') {
          el.style.setProperty('display', 'block', 'important');
    ...
    window.addEventListener('resize', () => {
      ...
      fixTimeout = setTimeout(fixStickyParents, 300);
    });
    
    return () => {
      clearTimeout(fixTimeout);
      window.removeEventListener('resize', fixStickyParents);
    };
  }, []);
};
